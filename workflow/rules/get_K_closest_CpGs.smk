rule get_K_closest_CpGs:
    ...
    conda:
        "../envs/bedtools.yml"
    log:
        "logs/get_K_closest_CpGs/get_K_closest_CpGs_{assembly_code}_v{gencode_version}_{tss_subset}_{minCount_expr}_{minSamples_expr}.log"
    shell:
        """
        bedtools closest -a {input.filtered_TSSs} -b {input.CpGs} -t all -k 5000 -d | awk -F '\t' 'BEGIN{{OFS="\t"}} {{print $5, $6, $7, $8, $4, $9}}' > {output} 2>{log}
        """
