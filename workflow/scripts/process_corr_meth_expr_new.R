...
output_path_cor                           <- args$output_path_cor

if (is_empty(input_path_corr) ||
    is_empty(input_path_sel_feat)) {

  write.table(data.frame(), file = output_path_cor, sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
...
colnames(corr) = c("CpG_ID", "correlation", "pval")
...
corr.f$score <- corr.f$correlation*(1-corr.f$pval)
...
bed <- data.frame(
  ...
  name  = corr.f$CpG_ID,
  correlation = corr.f$correlation,
  pval  = corr.f$pval,
  score = corr.f$score
)

write.table(bed, file = output_path_cor, sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
