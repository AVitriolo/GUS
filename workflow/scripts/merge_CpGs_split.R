...
TRAIN <- readLines(train_samples_path)              # <<< NEW
...
# ---- LEAKAGE-SAFE: the j-term is scored on TRAIN samples only --------------  <<< NEW
...
diag_df <- do.call(rbind, lapply(names(KNNs_list), function(k) {
    ...
    clusters <- r$clusters
    # common_samples is now TRAIN-only (defined above)          # <<< CHANGED
